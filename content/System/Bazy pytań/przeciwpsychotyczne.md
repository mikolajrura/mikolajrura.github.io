---
title: "Pytania — Leki przeciwpsychotyczne"
tags:
  - farmakologia
  - pytania
sekcja: przeciwpsychotyczne
---

> [!info] O tej notatce
> Pytania z bazy IV rok (ekstrakcja wg seminariów/nagłówków). Recall = „odsłoń odpowiedź". Kliknij w quizie: dobra → zielone, zła → czerwone z ❌.

> [!tip] Sekcja
> Leki: [[System/Sekcje leków/przeciwpsychotyczne|Leki przeciwpsychotyczne]].

### Pytanie 1

```quiz
Do charakterystycznych działań niepożądanych klasycznych neuroleptyków należą wszystkie w/w prócz
- Hiperprolaktynemia
- złośliwy zespół neuroleptyczny
- późne dyskinezy
- polekowy zespół Parkinsonowy
+ rabdomioliza (zespol objawow chorobowych wywolany rozpadem tkanki miesniowej)
```

---
### Pytanie 2

```quiz
Wskaż pary leków, które należą do atypowych neuroleptyków: **
+ Klozapina i kwetiapina
- haloperidol i chlorpromazyna
- arypiprazol i olanzapina
- flupentyksol i perazyna
- a i c prawidłowe
```

---
### Pytanie 3

```quiz
Blokowanie przez neuroleptyki którego z wymienionych receptorów powoduje osłabienie negatywnych objawów psychoz i zmniejszenie działań ze strony układu pozapiramidowego?
- d2 i d3 słabo d1 i d5
- alfa 1 i histaminowe
- M1
+ 5HT2
- M1 i 5HT2
```

---
### Pytanie 4

```quiz
Wskaż charakterystyczne działania farmakologiczne chlorprotyksenu
- uspokajające i przeciwlękowe
- przeciwhistaminowe i przeciwwymiotne
- zwiększa stężenie prolaktyny w wyniku zablokowania prolaktynostatyny
- przeciwwytwórcze i przeciwautystyczne
+ wszystkie wymienione
```

---
### Pytanie 5

```quiz
Wskaż fałszywe stwierdzenie charakteryzujące haloperidol
- działa silnie przeciwpsychotycznie
- może działać depresjotworczo
- zwiększa wydzielanie prolaktyny
+ działa silnie aktywizująco
- wszystkie wymienione
```

---
### Pytanie 6

```quiz
Konsekwencjami wzrostu stężenia prolaktyny przy długotrwałej terapii neuroleptykami są wszystkie z wyjątkiem
- Powiększenie piersi
- mlekotok
- wtórny brak miesiączki
- spadek libido
+ niedoczynność tarczycy
```

---
### Pytanie 7

```quiz
Lek X działa głównie przez blokadę postsynaptycznych receptorów D2. Powoduje wzrost wydzielania gonadotropin, hormonu wzrostu i prolaktyny, a ponadto jest odpowiedzialny za depresję ośrodka sytości. Lek X to: **
+ chlorpromazyna
- ziprazidon
- kwetiapina
- haloperidol
- klozapina
```

---
### Pytanie 8

```quiz
Do działań niepożądanych neuroleptyków, zw z zaburzeniami ze strony układu pozapiramidowego, należą
- mlekotok, wielomocz
- hamowanie wydzielania hormonu wzrostu, adh, gonadotropin
+ dyskinezy, akatyzja
- zaburzenia widzenia, suchość w jamie ustnej, hipotermia
- nadwrażliwość na światło, żółtaczka cholestatyczna
```

---
### Pytanie 9

```quiz
Wskaż fałszywe zestawienie receptor charakterystyka
- D2 i D3 mogą być autoreceptorami hamującymi uwalnianie DA
- D1 aktywują cyklaze adenylanową i zwiększają aktywność neuronów OUN
+ D2 i D3 mogą być autoreceptorami pobudzającymi uwalnianie DA
- D1 wpływają na kanały potasowe i wapniowe
- D1 i D2 związane są m.im. z mechanizmami wzmocnienia i nagrody
```

---
### Pytanie 10

```quiz
Do atypowych neuroleptyków nie należy
- risperidon
- olanzapina
- kwetiapina
+ haloperidol
- klozapina
```

---
### Pytanie 11

```quiz
Hipotensja, uspokojenie, senność to charakterystyczne objawy blokowania przez neuroleptyki następujących par receptorów
- D2 i D3 słabo D1 i D5
+ adrenergicznych alfa1 i histaminowych H1
- D2 i 5-HT2
- M1 i 5-HT2
- D2 i D3
```

---
### Pytanie 12

```quiz
Neuroleptyki mogą wykazywać wszystkie wymienione działania farmakologiczne z wyjątkiem
- przeciwwymiotne
- przeciwbólowe
- drgawkotwórcze
+ hipertermiczne
- przeciwwytwórcze
```

---
### Pytanie 13

```quiz
Wskaż prawidłową informację dot. pochodnych fenotiazyny
- blokują rec. D2, 5-HT2, a1, M1, H1
- stosowane w schizofrenii z nadpobudliwością ruchową
- działają przeciwpsychotycznie, uspokajająco, przeciwlękowo
- przeciwwskazane w chorobie wrzodowej
+ wszystkie prawidłowe
```

---
### Pytanie 14

```quiz
Lek X skutecznie łagodzi objawy pobudzenia i lęk u małych dzieci i osób starszych. Wykorzystywany w leczeniu zaburzeń osobowości, zależności alkoholowej, w premedykacji przed zabiegami chirurgicznymi w celu uspokojenia chorego oraz w celu zapobiegania wymiotom pooperacyjnym. Jest to
- haloperidol
- risperidon
- sulpiryd
+ chlorprotyksen
- olanzapina
```

---
### Pytanie 15

```quiz
Do działań niepożądanych klozapiny należą wszystkie wymienione objawy z wyjątkiem
+ nasilona hipoglikemia
- gorączka *(bo może powodować złośliwy zespół neuroleptyczny, więc poprawna)*
- napady drgawkowe
- agranulocytoza
- uszkodzenie mięśnia sercowego
```

---
### Pytanie 16

```quiz
Wskaż fałszywe twierdzenie opisujące kwetiapinę
- stosowana w manii i depresji w przebiegu ChAD
+ powoduje silny zespół parkinsonowski
- z objawów pozapiramidowych powoduje głównie akatyzję
- bardzo często występują objawy odstawienia
- powoduje przykre sny i koszmary senne
```

---
### Pytanie 17

```quiz
Do działań niepożądanych neuroleptyków zw z hamowaniem neuroprzekaźnictwa DA, NA, ACh w OUN należą
- mlekotok, wielomocz (retencja wody)
- hamowanie wydzielania hormonu wzrostu, ADH, gonadotropin (wzrost)
- dyskinezy, akatyzja
- zaburzenia widzenia, suchość w jamie ustnej, hipotermia
+ prawidłowe wszystkie oprócz D (a nie tylko c? jest retencja moczu)
```

---
### Pytanie 18

```quiz
Które rec. dopaminowe wpływają na kanały potasowe oraz wapniowe i związane są .in. z mechanizmami wzmocnienia i nagrody
- D5
+ D2
- D3
- D4
- wszystkie wyżej wymienione
```

---
### Pytanie 19

```quiz
Wskaż fałszywe dot. pochodnych fenotiazyny
- blokują rec D2, 5-HT2, a1, M1, H1
- stosowane w schizofrenii z nadpobudliwością ruchową
- działają przeciwpsychotycznie, uspokajające, p.lękowo
+ stosowane w terapii choroby wrzodowej
- wszystkie prawidłowe
```

---
### Pytanie 20

```quiz
Flupentyksol w większych dawkach działa: **
+ aktywizująco i przeciwlękowo
- przeciwpsychotycznie i przeciwautystycznie
- uspokajająco i przeciwwymiotnie
- prawidłowe A i C (A nie a i b? Mam w notatkach że flupentyksol nie uspokaja)
```

---
### Pytanie 21

```quiz
Lek X należy do najsilniejszych neuroleptyków z grupy pochodnych butyrofenonu. W schizofrenii działa silnie na objawy wytwórcze. U osób pobudzonych zmniejsza napęd psychoruchowy i niepokój. ma także działanie przeciwwymiotne oraz skutecznie hamuje nudności i czkawkę. Jest to
- chlorprotyksen
+ haloperidol
- sulpiryd
- kwetiapina
- chloropromazyna
```

---
### Pytanie 22

```quiz
Działanie niepożądane neuroleptyków zw są z blokadą wielu rodzajów receptorów. Wskaż prawidłowe działanie niepożądane następujące po zablokowaniu wymienionego receptora
- 5-HT3 - obj. pozapiramidowe
- M1 - hiperprolaktynemia
+ alfa-1 - niedociśnienie
- D2 - zwiększenie masy ciała
- wszystkie prawidłowe
```

---
### Pytanie 23

```quiz
Cechami charakterystycznymi neuroleptyków atypowych są wszystkie oprócz
- mniej działań niepożądanych w porównaniu z klasycznymi
- rzadziej wywołują hiperprolaktynemię
- działają na pozytywne i negatywne objawy schizofrenii
+ są skuteczne w ostrych zaburzeniach psychotycznych
- wszystkie wymienione są cechami charakterystycznymi dla n.atypowych
```

---
### Pytanie 24

```quiz
Do zaburzeń ze strony układu pozapiramidowego, możliwych po neuroleptykach, należą wszystkie oprócz: **
+ akatyzje(?)
- polekowy zespół Parkinsonowi (?)
- złośliwy zespół neuroleptyczny (?)
- dyskinezy(?)
- hiperprolaktynemia
```

---
### Pytanie 25

```quiz
Wskaż fałszywe twierdzenie opisujące arypiprazol
- częściowy agonista receptorów 5-HT2 i D2
+ antagonista receptorów 5-HT3
- „stabilizator układu DA/5-HT"
- stosowany w umiarkowanej i ciężkiej manii w przebiegu choroby afektywnej dwubiegunowej
- wszystkie twierdzenia są prawdziwe
```

---
### Pytanie 26

```quiz
Działania niepożądane leku X są związane z blokadą rec. D2. Dodatkowo może wystąpić żółtaczka zastoinowa i podciśnienie ortostatyczne. Lekiem X jest
- Klozapina
- Olanzapina
- Sulpiryd
+ Chloropromazyna
- Kwetiapina
```

---
### Pytanie 27

```quiz
O haloperidolu prawdziwe są wszystkie twierdzenia z wyjątkiem
- Silnie hamuje nudności i czkawkę
- Wykazuje umiarkowane działanie przeciwwymiotne
- Wywołuje silne objawy pozapiramidowe
+ wykazuje działanie przeciwdepresyjne
- wszystkie odpowiedzi są prawidłowe
```

---
### Pytanie 28

```quiz
Wybrać z podanych neuroleptyk o najsłabszym działaniu przeciwpsychotycznym
- haloperidol
+ chlorprotyksen
- flupentyksol
- Lewopromazyna
- sulpiryd
```

---
### Pytanie 29

```quiz
Wskaż prawidłowe zestawienie lek - działanie/zastosowanie
- Chlorprotyksen - głównie działa przeciwlękowo
- Haloperidol - zmniejszenie napędu psychoruchowego
- Klozapina - długotrwała terapia może powodować rozwój białaczki
- Sulpiryd - stosowany m.in. w leczeniu zawrotów głowy i migren
+ Wszystkie odpowiedzi są prawidłowe
```

---
### Pytanie 30

```quiz
Wskaż prawidłowe informacje dotyczące pochodnych fenotiazyny
- Są metabolizowane przez cytochrom P450
- Wykazują antagonizm wobec receptorów H1, alfa 1, 5-HT2, M i D2
- Przenikają przez barierę krew-mózg i łożysko
- Wykazują umiarkowane działanie sprzeciw autystyczne
+ Wszystkie informacje są prawidłowe
```

---
### Pytanie 31

```quiz
Które z wymienionych działań jest odpowiedzialne za zmniejszenie tzw. objawów pozytywnych schizofrenii
- pobudzenie receptorów dopaminowych ośrodkowych
- obniżenie progu pobudliwości drgawkowej
+ blokowanie ośrodkowych receptorów dopaminowych
- blokowanie obwodowego przekaźnictwa serotoninowego
- działanie cholinolityczne ośrodkowe
```

---
### Pytanie 32

```quiz
Neuroleptykiem o niewielkim działaniu antypsychotycznym w porównaniu z pozostałymi wymienionymi lekami jest
- Haloperidol
+ Sulpiryd
- Chlorprotyksen
- Flupentyksol
- Lewomepromazyna
```

---
### Pytanie 33

```quiz
Który lek z grupy pochodnych tioksantenu ma silniejsze działanie aktywizujące i przeciwautystyczne
- Klopentiksol
- Chlorprotiksen
+ Flupentiksol
- Haloperidol
- Chloropromazyna
```

---
### Pytanie 34

```quiz
Do klasycznych neuroleptyków nie należy
- Chlorpromazyna
- Chlorprotiksen
- Sulpiryd
- Haloperidol
+ Kwetiapina
```

---
### Pytanie 35

```quiz
W małych dawkach klasyczne neuroleptyki mogą być podawane
- w stanach lękowych
- w premedykacji znieczulenia ogólnego
- w neuroleptanalgezji
- jako leki p/wymiotne
+ wszystkie odpowiedzi są poprawne
```

---
### Pytanie 36

```quiz
Wszystkie poniższe twierdzenia odnoszą się do klozapiny są prawdziwe, z wyjątkiem
- wybiórczo blokuje receptory dopaminergiczne D4
- wykazuje silne działanie uspokajające i p/lękowe
- może powodować zmiany w obrazie krwi
+ silnie pobudza wydzielanie prolaktyny
- stosowana w psychozach przy nieskuteczności innych neuroleptyków
```

---
### Pytanie 37

```quiz
Działanie p/wytwórcze, p/omamowe blokowanie
- D2 i muskarynowe
- M1 i 5-HT2
- D2 i 5HT2
- Adrenergiczne i H1
+ rec. D2 i D3, słabo D1 i D5
```

---
### Pytanie 38

```quiz
Działanie niepożądane chloropromazyny hipotonia ortostatyczna, wynika z
- Działanie cholinergiczne obwodowe
- Działanie cholinergiczne ośrodkowe
- Blok rec 5HT2
+ Działanie alfa1-adrenolityczne
- Działanie przeciwhistaminowe
```

---
### Pytanie 39

```quiz
O haloperidolu są prawdziwe za wyjątkiem
- Silnie hamuje nudności i czkawke
- Wykazuje umiarkowanego działanie przeciwwymiotne
- Wykazuje silne objawy pozapiramidowe
- Stosowany w manii i pobudzeniu psychoruchowym
+ Wszystkie odpowiedzi są prawdziwe
```

---
### Pytanie 40

```quiz
Do działań niepożądanych klozapiny należą wszystkie z wyjątkiem
- Gorączka
- Agranulocytoza
+ Bradykardia??
- Napady drgawkowe Wymień 3 działania niepożądane klasycznych neuroleptyków związane z hamowanie NA, ACh itd w OUN: ● Drgawki ● Hiperprolaktynemia ● Hamowanie wydzielania hormonu wzrostu ● Działanie cholinolityczne
```

---

## Pytania z samą odpowiedzią (recall)

### Recall 1

Wskaż parę neuroleptyków klasycznych

> [!success]- Odpowiedź
> haloperydol i chlorprotyksen

---
### Recall 2

Atypowy neuroleptyk, stosowany w schizofrenii opornej, powoduje agranulocytozę

> [!success]- Odpowiedź
> klozapina

---
### Recall 3

Pytanie o lewomepromazynę, wybrać fałszywe

> [!success]- Odpowiedź
> słabe działanie przeciwbólowe

---
### Recall 4

Blokowanie którego receptora wpływa na objawy negatywne i zmniejszenie skutków ubocznych?

> [!success]- Odpowiedź
> 5-HT2

---
### Recall 5

Działanie przeciwautystyczne posiada?

> [!success]- Odpowiedź
> flupentiksol

---
### Recall 6

Lek o najsilniejszym działaniu przeciwautystycznym, dodatkowo stosowany w bólach głowy o różnej etiologii, w migrenach i leczeniu uzależnienia alkoholowego to

> [!success]- Odpowiedź
> sulpiryd

---
### Recall 7

Wskaż błędna parę reptor-efekt

> [!success]- Odpowiedź
> - M2-zwiększone wydzielanie prolaktyny

---
### Recall 8

Hipotensja, senność i uspokojenie. Zaznaczyć pary receptorów. Odp

> [!success]- Odpowiedź
> Alfa-1 i H-1

---
### Recall 9

Jakie receptory zmniejszają poziom cAMP

> [!success]- Odpowiedź
> D2,3,4

---
### Recall 10

Wybrać fałszywe o atypowych

> [!success]- Odpowiedź
> stosowane w ostrych stanach psychotycznych

---
### Recall 11

lewomepromazyna prawdziwe wszystkie za wyjątkiem

> [!success]- Odpowiedź
> wykazuje SŁABE działanie przeciwbólowe (bo wykazuje SILNE)

---
### Recall 12

sulpiryd fałszywe są wszystkie za wyjątkiem

> [!success]- Odpowiedź
> działa przeciwwymiotnie

---
### Recall 13

Autoreceptory

> [!success]- Odpowiedź
> -- D2 i D3

---
### Recall 14

Zestawienie receptor -- czynność

> [!success]- Odpowiedź
> alfa 1 -- niskie ciśnienie

---
### Recall 15

Do działań neuroleptyków należą wszystkie poza

> [!success]- Odpowiedź
> działaniem psychotwóczym

---
### Recall 16

Słabe działanie w porównaniu z pozostałymi wykazuje

> [!success]- Odpowiedź
> chlorprotyksen

---
### Recall 17

Silne działanie związku pochodnego tioksantentu

> [!success]- Odpowiedź
> flupentyksol

---
### Recall 18

Wskaż fałszywe zestawienie receptor-charakterystyka

> [!success]- Odpowiedź
> D2 to autorecpetor pobudzający wydzielanie dopaminy

---
### Recall 19

Do neuroleptyków atypowych nie należy

> [!success]- Odpowiedź
> Haloperidol

---
### Recall 20

Do działań niepożądanych ze strony układu pozapiramidowego należą

> [!success]- Odpowiedź
> akatyzje, diskinezy, złośliwy zespół neuroleptyczny, zespół parkinsonowski, ogólnie wszystko oprócz nietrzymania moczu

---
### Recall 21

Wpisać nazwę leku, atypowego neuroleptyku, częściowego agonisty D2

> [!success]- Odpowiedź
> aripiprazol

---
### Recall 22

Wybrać dobrze dopasowaną parę receptor - działanie niepożądane neuroleptyków

> [!success]- Odpowiedź
> a1 - niedociśnienie

---


## Pytania dodatkowe (z routingu — zweryfikuj)

### Dodatkowe 1

```quiz
Buspiron to
- atypowy neuroleptyk
- cholinolityk o silnym działaniu spazmolitycznym
- lek anksjolityczny, antagonista receptora GABA-A
- silny inhibitor receptora 5-HT1
+ lek p/lękowy, częściowy agonista receptora serotoninergicznego 5-HT1A
```

---
### Dodatkowe 2

```quiz
Do działań niepożądanych L-dopy należy/ą
- zaburzenia psychotyczne w tym wyobrażenia i urojenia paranoidalne
- dyskinezje
- nudności i hipotonia ortostatyczna
- odpowiedzi B i C są prawidłowe
+ wszystkie są prawidłowe
```

---
### Dodatkowe 3

```quiz
Biperiden
- jest przeciwwskazany w niedrożności jelit
- to cholinolityk ze szczególnym powinowactwem do rec. M1
- może być stosowany w zespołach pozapiramidowych wywołanych przez neuroleptyki
- jednym z działań niepożądanych jest zaburzenie akomodacji
+ wszystkie prawidłowe
```

---
### Dodatkowe 4

```quiz
Neuroleptyki stosowane w znoszeniu niektórych objawów psychotycznych po długotrwałym stosowaniu LEWODOPY zmniejszają skuteczność leków
- cholinolitycznych
- adrenolitycznych
+ dopaminergicznych
- GABA-ergicznych
```

---
### Dodatkowe 5

```quiz
Które leki mogą zmniejszać skuteczność agonistów dopaminergicznych stosowanych w chorobie Parkinsona
- chloropromazyna
- olanzapina
- klozapina
+ wszystkie
```

---
### Dodatkowe 6

```quiz
Wskaż objawy które nie występują po długotrwałej farmakoterapii ch. Parkinsona lek. dopaminergicznymi
- spadek wydzielania prolaktyny
- wzrost wydzielania ACTH
+ wzrost wrażliwości receptorów D2
- stany psychotyczne
- dyskinezy
```

---
### Recall dod. 1

Zaznacz działania benzodiazepin

> [!success]- Odpowiedź
> wszystkie bez psychotycznego

---
### Recall dod. 2

W neuroleptanalgezji stosujemy

> [!success]- Odpowiedź
> fentanyl i droperidol

---
### Recall dod. 3

Działania niepożądane pozapiramidowe klasycznych neuroleptyków, które do nich

> [!success]- Odpowiedź
> nie należą

---
### Recall dod. 4

które leki nie należą do neuroleptyków atypowych

> [!success]- Odpowiedź
> haloperidol

---
### Recall dod. 5

które z działań niepożądanych neuroleptyków klasycznych

> [!success]- Odpowiedź
> nie jest ośrodkowe

---
### Recall dod. 6

lek działa przeciwpsychotycznie i w małych dawkach sedatywnie (?), jaki to lek odpowiedzi: na pewno był w odpowiedzial

> [!success]- Odpowiedź
> arypiprazol

---


## Egzamin 2-termin 13.07.2023 (dodane)

### 2T2023 · 1

Z BAZY: Klasyczne neuroleptyki mogą wywoływać wszystkie niżej wymienione działania niepożądane, oprócz

> [!success]- Odpowiedź
> a.

---
